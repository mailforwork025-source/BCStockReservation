permissionset 60100 "BCSR ADMIN"
{
    Assignable = true;
    Caption = 'BC Stock Reservation Admin';

    Permissions =
        tabledata "BCSR Setup" = RIMD,
        tabledata "BCSR Availability Bucket" = RIMD,
        tabledata "BCSR Reservation Header" = RIMD,
        tabledata "BCSR Reservation Line" = RIMD,
        tabledata "BCSR Operation Log" = RIMD,
        tabledata "BCSR Failure Queue" = RIMD,
        tabledata "BCSR Audit Log" = RIMD,
        tabledata "BCSR Backorder Header" = RIMD,
        tabledata "BCSR Backorder Line" = RIMD,
        tabledata "BCSR Schema Version" = RIMD,
        tabledata "BCSR Backorder Cue" = RIMD,
        tabledata Item = RIMD,
        table "BCSR Setup" = X,
        table "BCSR Availability Bucket" = X,
        table "BCSR Reservation Header" = X,
        table "BCSR Reservation Line" = X,
        table "BCSR Operation Log" = X,
        table "BCSR Failure Queue" = X,
        table "BCSR Audit Log" = X,
        table "BCSR Backorder Header" = X,
        table "BCSR Backorder Line" = X,
        table "BCSR Schema Version" = X,
        table "BCSR Backorder Cue" = X,
        page "Stock Reservation Setup" = X,
        page "Active Reservations" = X,
        page "Reservation History" = X,
        page "BCSR Reservation Card" = X,
        page "BCSR Reservation Lines" = X,
        page "BCSR All Reservation Lines" = X,
        page "Backorder List" = X,
        page "BCSR Backorder Header List" = X,
        page "BCSR Backorder Card" = X,
        page "BCSR Backorder Lines Part" = X,
        page "BCSR Backorder Activities" = X,
        page "Inventory Avail. Dashboard" = X,
        page "BCSR Failure Queue" = X,
        page "BCSR Operation Log" = X,
        page "BCSR Audit Log" = X,
        page "BCSR Reservations API" = X,
        page "BCSR Reservation Lines API" = X,
        page "BCSR Availability API" = X,
        page "BCSR Backorders API" = X,
        page "BCSR Backorder Lines API" = X,
        page "BCSR Operations API" = X,
        page "BCSR Failures API" = X,
        page "BCSR Delivery Timeline Card" = X,
        page "BCSR Delivery Timelines" = X,
        page "Bundle Card" = X,
        page "Bundle List" = X,
        page "Bundle Shops Factbox" = X,
        page "Bundle Item Combinations Part" = X,
        page "Bundle Combination Header Part" = X,
        page "Bundle Combination Header Card" = X,
        page "Bundle Combination Lines Part" = X,
        page "Bundle Item Options Part" = X,
        page "Bundle Item Products Part" = X,
        page "Bundle Image Map Header Part" = X,
        page "Bundle Image Map Header Card" = X,
        page "Bundle Image Map Lines Part" = X,
        report "BCSR Bulk Enable Reservation" = X,
        report "BCSR Bulk Disable Reservation" = X,
        report "BCSR Bulk Enable Backorder" = X,
        report "BCSR Bulk Disable Backorder" = X,
        report "BCSR Bulk Enable Resrv. Only" = X,
        codeunit "BCSR Reservation Service" = X,
        codeunit "BCSR Backorder Service" = X,
        codeunit "BCSR API Actions" = X,
        codeunit "BCSR Availability Mgt." = X,
        codeunit "BCSR Idempotency Mgt." = X,
        codeunit "BCSR Audit Mgt." = X,
        codeunit "BCSR Telemetry Mgt." = X,
        codeunit "BCSR Test API Diag" = X,
        codeunit "BCSR Delivery Mgt." = X,
        codeunit "BCSR Upgrade Mgt." = X,
        codeunit "Reservation Install Mgt." = X,
        codeunit "Reservation Job Handler" = X,
        codeunit "Reservation Setup Mgt." = X,
        codeunit "BCSR Bundle Combo Gen." = X;
}

permissionset 60101 "BCSR API"
{
    Assignable = true;
    Caption = 'BC Stock Reservation API';

    Permissions =
        tabledata "BCSR Setup" = R,
        tabledata "BCSR Availability Bucket" = RIMD,
        tabledata "BCSR Reservation Header" = RIMD,
        tabledata "BCSR Reservation Line" = RIMD,
        tabledata "BCSR Operation Log" = RIMD,
        tabledata "BCSR Failure Queue" = RIMD,
        tabledata "BCSR Audit Log" = RIMD,
        tabledata "BCSR Backorder Header" = RIMD,
        tabledata "BCSR Backorder Line" = RIMD,
        tabledata "Bundle Header" = RIMD,
        tabledata "Bundle Item Option" = RIMD,
        tabledata "Bundle Item Product" = RIMD,
        tabledata "Bundle Item Product V2" = RIMD,
        tabledata "Bundle Item Combination" = RIMD,
        tabledata "Bundle Combination Header" = RIMD,
        tabledata "Bundle Combination Line" = RIMD,
        tabledata "Bundle Image Mapping Header" = RIMD,
        tabledata "Bundle Image Mapping Line" = RIMD,
        tabledata "BCSR Delivery Timeline" = RIMD,
        page APIBundles = X,
        page APIBundleComponents = X,
        page APIBundleOptions = X,
        page APIBundleCombinationHeaders = X,
        page APIBundleCombinationLines = X,
        page APIBundleImageMappingHeaders = X,
        page APIBundleImageMappingLines = X,
        codeunit "BCSR API Actions" = X,
        codeunit "BCSR Reservation Service" = X,
        codeunit "BCSR Backorder Service" = X,
        codeunit "BCSR Availability Mgt." = X,
        codeunit "BCSR Idempotency Mgt." = X,
        codeunit "BCSR Audit Mgt." = X,
        codeunit "BCSR Test API" = X,
        codeunit "BCSR Test API Diag" = X,
        codeunit "BCSR Delivery Mgt." = X,
        tabledata Item = RIMD;

    }
