codeunit 60104 "BCSR API Actions"
{
    ...
    [ServiceEnabled]
    procedure CalculateDelivery(ItemNo: Code[20]; Quantity: Decimal; IsBundle: Boolean; OptionsJson: Text): Text
    var
    ...
    // TEMPORARY diagnostic action - reads Sales Lines directly via AL rather
    // than BC's native /api/v2.0 salesOrderLines endpoint (blocked, 403, in
    // this tenant), so the bundle-split fix can be verified without relying
    // on the BC web client's own refresh/caching behavior. Safe to remove
    // once the split is confirmed working end-to-end.
    [ServiceEnabled]
    procedure DebugGetSalesOrderLines(bcSalesOrderNo: Code[20]): Text
    var
    ...
    begin
        if not SalesHeader.Get(SalesHeader."Document Type"::Order, bcSalesOrderNo) then
            exit('{"success":false,"message":"Sales order not found."}');

        ResponsePayload := '{"success":true,"status":"' + Format(SalesHeader.Status) + '","lines":[';
        First := true;
        ...
        if SalesLine.FindSet() then
            repeat
                ...
                ResponsePayload +=
                    ...
                    '"no":"' + Format(SalesLine."No.") + '",' +
                    '"description":"' + Format(SalesLine.Description) + '",' +
                    '"quantity":' + Format(SalesLine.Quantity, 0, 9) +
                    '}';
    ...
    // TEMPORARY diagnostic action - reads the DebugSplitTrace audit entries
    // written inside EnsureBundleSalesLinesSplit, so the exact branch taken
    // during a real ConfirmSync run can be seen directly instead of inferred
    // from the resulting Sales Order state. Remove alongside the trace calls
    // once the split issue is resolved.
    [ServiceEnabled]
    procedure DebugGetAuditTrace(reservationId: Guid): Text
    var
        AuditLog: Record "BCSR Audit Log";
        ResponsePayload: Text;
        First: Boolean;
    begin
        AuditLog.SetRange("Entity ID", reservationId);
        AuditLog.SetRange("Event Type", 'DebugSplitTrace');
        AuditLog.SetCurrentKey("Entity ID", "Created DateTime");
        ResponsePayload := '{"success":true,"entries":[';
        First := true;
        if AuditLog.FindSet() then
            repeat
                ...
                First := false;
                ResponsePayload +=
                    '{"createdDateTime":"' + Format(AuditLog."Created DateTime", 0, 9) + '",' +
                    '"message":"' + Format(AuditLog.Message) + '"}';
